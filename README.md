...
- 生效规则：面板内所有配置修改后**立即保存并即时生效**，无需重启——
  KVM 开关热启停（监听与会话即时开闭）、节点名下一次广播（约几秒）生效、
  阈值/接收目录/自动回贴/文本同步/跨屏手感（合拍间隔/重排节拍/速度系数/
  邻居布局）即时生效
...
| `kvm_move_interval_ms` | 0（=8） | 主控移动合拍间隔（ms），越小越跟手但更耗带宽；0 用默认 8（≈125Hz）。**面板保存后立即生效** |
| `kvm_reflow_step_ms` | 0（=4） | 被控重排注入节拍（ms），0 用默认 4（≈250Hz），`-1` 关闭重排恢复逐包直注。**面板保存后立即生效** |
| `kvm_speed_percent` | 100 | 本机作主控时发送位移的手调系数百分比（100=不补偿；越大远程越快）。解决"远程与本机快慢不一致"：每台机器各设一档，A→B 偏慢调高 A、B→A 偏慢调高 B；**面板保存后立即生效** |
...
取消某侧（✕）或把该侧换成其他设备时同样自动同步——旧邻居会收到取消通知并
移除对应配置，不会残留过期邻居。手工编辑 config.json 等价（只需改一端，
另一端靠同步；注意手工改完重启本进程后还会被面板布局覆盖）：
