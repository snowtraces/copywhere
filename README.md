...
| `kvm_speed_percent` | 100 | 本机作主控时速度微调倍率百分比（100=自动计算的标准速度；可在 50~300% 个性化微调）；**面板保存后立即生效** |
...
- 在 A 上把光标推向**右边缘并继续推**（累计约 8px，轻推即过）→ 控制权切到 B，键盘/滚轮同步转发；
  在 B 上把光标推回**左边缘并继续推**（累计约 8px）→ 交还 A；200ms 防抖消除卡顿，往返穿梭流畅自如
- **被控期间在本机动一下鼠标或键盘 → 会话自动结束**（真人回到本机操作）
- 切换/切回只在**暴露边缘**（该侧没有相邻本地显示器）触发，双屏内侧边界不会误判；
  被控入口固定在主显示器（`kvm_entry_monitor` 可指定入口屏）
- **跨分辨率与实际 DPI 缩放自适应**：系统自动读取两端真实物理分辨率与 Windows 实际设置缩放比（如 ThinkBook 2880×1800 @ 200%），按逻辑视口换算科学基准增益并结合亚像素余数结转，解决高分缩放屏变慢问题，绝不随手速漂移；被控多屏自由漫游；面板支持速度微调滑块（默认 100%）
