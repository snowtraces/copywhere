...
- **图形界面（托盘 + 面板）**：`gui` 同进程启动服务 + 系统托盘 + 浏览器控制面板，
  传输动态为主页（收发时间线 + 实时进度），全局拖拽投送、卡片式节点列表、
  日志抽屉、侧滑设置、深浅双主题，SSE 实时刷新，仅监听 127.0.0.1
...
1. 在两台机器上各自生成默认配置（首次运行 `run`/`gui` 也会自动生成）：

   ```powershell
   .\copywhere.exe init
   ```

   建议顺便改一个友好的 `node_name`（面板设置页也可改）。

2. **配对**：启动 `gui`，在「在线节点」页对发现的对方节点点击「配对」，
   对端面板弹出「接受配对」确认后即完成。未配对的节点无法传输。
...
`copywhere gui` 在同一进程内启动完整服务（发现/传输/剪贴板/KVM），并附加：

- **系统托盘**：打开面板、暂停/恢复自动同步、打开接收目录、退出
- **浏览器面板**（`http://127.0.0.1:47890`，由托盘自动打开）：
...
- **配对认证**：A 端点击「配对」→ B 端面板弹出「接受配对」确认 → 完成。
  双方交换**逐对独立**的配对令牌，保存在 `~/.copywhere/peers.json`；
  解除配对即吊销。共享 token 认证已废弃，未配对节点无法传输。
- **屏幕布局跨机同步**：在一端拖动配置 KVM 左右邻后，布局自动推送到对端
  （互为镜像），双方即时热生效，无需在两台机器上重复配置、也无需重启。
- **安全**：面板仅监听 `127.0.0.1`，首次经托盘 URL 带随机访问令牌并种下 cookie；
